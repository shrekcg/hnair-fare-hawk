...
- [x] **监控任务航线/起降时间加粗加深（2026-09-04，纯前端）**：
  - 监控任务表航线列：航班号 600→700、城市名加粗（继承 650）、起降时间 mono 400→650 且显式 `--color-ink`，突出数据重点；仅改该列，总览/查询其他表格不动；浅色/暗色同步生效。
  - 验证：pnpm build OK（index-BsaoFcvD.js）、Playwright 浅/暗实测字重 700/650 与颜色正确。提交见 Git 历史。
