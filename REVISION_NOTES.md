...
## 三、优化方案清单（2026-09-02 批次1 已落地，提交 67086af）

| # | 优化 | 状态 |
|---|---|---|
| 1 | 启动绑定 `127.0.0.1` | ✅ 已修复（start_all.sh / stop_all.sh 固化，Web 实测仅监听 127.0.0.1:8501） |
| 2 | 任务级错峰（10–30s 随机） | ✅ 已实现（daemon 任务间随机等待） |
| 3 | 认证失败指数退避 | ✅ 已实现（按任务 5min→60min，持久化 runtime_state；鉴权失败不再中断整轮） |
...
| 11 | 页面暴露任务 enabled 开关 | ✅ 已实现（任务列表启停按钮，daemon 侧原有过滤保留） |
