 ...
 - **底表（航线 / 班期 / 可飞日期 / 档位规则）**为本地静态数据资产，来源于海航随心飞官方产品信息（2026 航季 666 / 2666 会员专享档位页面）与上游公开整理的航班计划 CSV，项目对原始数据做了**二次加工**：
   - 两条源以（航班号、出发城市、到达城市）为键合并归一化，产品档位取两源并集（宁多勿漏），两源判定不一致时标记 `product_conflict` 供人工复核；
   - 可飞日期优先采用官方页面实际执飞区间，CSV 备注（如「9.28始」「10.8止」「仅9.9」）解析为修正后的日期区间；
   - 第三方校正只更新起降时刻 / 航站楼 / 经停等基础事实，不覆盖航线集合、班期、可飞日期与档位。
 
   加工产物：`data/sediment/flights_normalized.json`（1733 条）；档位×日期屏蔽规则见 `data/sediment/tier_block_rules.json`（单一事实源）。
 
 - **价格 / 经停 / 余票**非底表数据，由官方接口**实时查价**，仅作个人监控参考，绝不回写底表。
 
 ## 限制与合规
 
 - 仅本机运行，绑定 127.0.0.1，不做公网暴露。
- 实时查价依赖你**自行抓取的官方接口票据**；接口与签名实现随项目提供，但票据与令牌需由你自行抓取维护。本项目不提供任何绕过验证或风控的手段。
 - 底表静态数据来源于公开产品信息整理并经过本地二次加工（见[数据来源](#数据来源)），价格与余票仅作个人监控参考。
